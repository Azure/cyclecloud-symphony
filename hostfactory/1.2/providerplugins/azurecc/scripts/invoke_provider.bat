...
if exist "%dirname%\..\.venv\" (
    call "%dirname%\..\.venv\azurecc\Scripts\activate.bat"
    set embedded_python="%dirname%\..\.venv\azurecc\Scripts\python.exe" 
) else (
...
cmd /c "%embedded_python% -m cyclecloud_provider %*" > %TEMP%\azurecc_invoke.out.log 2>%TEMP%\azurecc_invoke.err.log
